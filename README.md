...
- ⏱ 专注计时：顶部实时计时徽章 + 专注面板（每次记录 / 今天 / 本周累计），手动开始/停止，退出自动记录，可选到点提醒
- 🕘 历史记录 + 草稿自动保存 + **版本快照**（自动保存最近 30 个版本，可恢复），支持**导出 / 导入备份**（含历史、草稿、知识库），拖拽打开 `.md` 文件，或「载入文本」直接粘贴 Markdown 替换当前内容
- 📑 **多标签编辑**：打开的文件 / 载入的文本 / 知识库文档都以标签形式并存，可切换、关闭、新建，互不覆盖；**标签刷新后保留**（持久化）
- 🔖 **知识库批量**：多选 → 批量删除 / 导出 ZIP / 移动分类
...
- 导出 Word 优先生成原生 `.docx`（docx 库，保留标题/加粗/颜色/表格/图片）；若转换失败自动回退 `.doc`（HTML 包装）
