...
- 🧭 导航窗格：左侧显示文档标题大纲（H1~H6），**随阅读位置自动高亮当前所在章节**，点击精准跳转（长文档亦不跑偏）
...
- 🖨 **PDF 导出可调**：纸张（A4 / A3 / Letter）、边距、正文字号（五号 / 小四 / 四号）、行距，预览实时反映；导出自动优化分页（表格/图片/引用不被拦腰截断、标题不与正文分离、消除孤行）
...
- 🕘 历史记录 + 草稿自动保存 + **版本快照**（按存储空间自动保留最近若干份完整版本，可恢复），支持**导出 / 导入备份**（含历史、草稿、知识库），拖拽打开 `.md` 文件，或「载入文本」直接粘贴 Markdown 替换当前内容
- 📑 **多标签编辑**：打开的文件 / 载入的文本 / 知识库文档都以标签形式并存，可切换、关闭、新建，互不覆盖；**标签刷新后保留**（持久化）；**右键标签可重命名 / 复制标题 / 存入知识库 / 关闭**
- ⿻ **双文档分屏**：独立的并排对照界面（左主右副），两栏都是干净内容白板、不占导航/批注栏，中间可拖拽调比例，**一键互换主副**，退出自动保存改动；**记住分屏比例与上次对照组合**（下次点分屏直接恢复），每栏标题栏有「换文档」下拉可当场切换该栏文档
...
- 导出 PDF 依赖浏览器打印（另存为 PDF）；导出面板可选纸张 / 边距 / 字号 / 行距，选择会被记住。**页码**请在打印对话框勾选「页眉和页脚」（浏览器自带页码能力，网页端无法直接写页码）
